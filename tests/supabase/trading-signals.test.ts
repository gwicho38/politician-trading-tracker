...
Deno.test("Confidence calculation - base confidence", () => {
  ...
  const calculateConfidence = (
    ...
    if (politicianCount >= 5) confidence += DEFAULT_WEIGHTS.REDACTED;
    else if (politicianCount >= 3) confidence += DEFAULT_WEIGHTS.REDACTED;
...
Deno.test("DEFAULT_WEIGHTS - has correct structure", () => {
  ...
  assertEquals(DEFAULT_WEIGHTS.REDACTED, 0.15);
