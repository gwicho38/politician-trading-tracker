...
class PipelineOrchestrator:
    ...
    def __init__(
        ...
        self.clean_stage = CleaningStage(
            remove_duplicates=True,
            strict_validation=strict_cleaning
        )

        self.normalize_stage = NormalizationStage(
            auto_create_politicians=True
        )
    ...
    async def run(
        ...
        try:
            ...
            self.logger.info(
                f"Ingestion complete: {ingest_result.metrics.records_output} records"
            )
            ...
            self.logger.info(
                f"Cleaning complete: {clean_result.metrics.records_output} records"
            )
