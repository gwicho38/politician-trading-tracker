...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let syncType = 'full_sync'
    let metadata: Record<string, unknown> = { source: 'webhook' }
    
    try {
      const body = await req.json()
      syncType = body.sync_type || 'full_sync'
      metadata = { ...metadata, ...body.metadata }
    } catch {
      // No body or invalid JSON, use defaults
    }

    console.log(`Starting ${syncType} via webhook...`)
    ...
    try {
      ...
      for (const admin of admins || []) {
        await supabase.from('notifications').insert({
          ...
          message: `Sync via webhook failed: ${errorMessage}`,
          type: 'error',
          read: false
        })
      }
